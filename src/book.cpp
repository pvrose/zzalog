...
extern debug_flag DEBUG_MOD_STATUS;
...
const std::string default_header_ = "ADIF File generated by " + APP_NAME + "\n";
	...
	if (book_type_ == OT_MAIN || book_type_ == OT_IMPORT) {
		...
		if (choice != CANCEL) {
			...
			if (filename.length() != 0) {
				...
				if (!input_.is_open() && NEW_BOOK) {
					// This is a new book so ignore that it doesn't open.
					status_->misc_status(ST_WARNING, "LOG: Creating a new logbook %s",
						target_filename_.c_str());
					set_recent_file(target_filename_);
					if (!book_->store_data(target_filename_, true)) {
						status_->misc_status(ST_ERROR, "LOG: Failed to create %s", target_filename_.c_str());
					}
					else {
						backup_file();
					}
				} else if (!reader->load_book(this, input_)) {
...
void book::delete_dirty_record(record* qso) {
	if (is_dirty_record(qso)) {
		if (zc_app::debug(DEBUG_MOD_STATUS)) {
