...
void cty_data::load_sources() {
	...
	merge_data();
	delete_data(import_);
	if (loaded) timestamps_[type_] = file_holder_->timestamp(FILE_COUNTRY_CLUB);
	...
	merge_data();
	delete_data(import_);
	if (loaded) timestamps_[type_] = file_holder_->timestamp(FILE_COUNTRY_CFILES);
	else timestamps_[type_] = std::chrono::system_clock::time_point::min();
	check_timestamp(type_, 7);
	type_ = DXATLAS;
	loaded = load_data(&filename);
	merge_data();
	delete_data(import_);
	if (loaded) timestamps_[type_] = file_holder_->timestamp(FILE_COUNTRY_DXATLAS);
	else timestamps_[type_] = std::chrono::system_clock::time_point::min();
